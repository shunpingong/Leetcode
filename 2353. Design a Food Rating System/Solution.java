class FoodRatings {
            ...
            if(cuisineMap.containsKey(cuisines[i])){
                ...
                PriorityQueue<Info> pq=new PriorityQueue<Info>(new Comparator<Info>(){
                    ...
                    public int compare(Info a,Info b){
                        ...
                        if(result==0){
                            //same rating, sort based off string
    ...
    //     prev.setRating(newRating);
    //     foodMap.put(food,prev);
    //     cuisineMap.get(prev.cuisine).add(prev);
    // }

    //shorter runtime because dont need to search and remove
    public void changeRating(String food, int newRating) {
        Info prev=foodMap.get(food);
